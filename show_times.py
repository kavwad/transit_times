...
def combine_lines(arrivals_nearby):
    """
    ...
    arrivals_nearby['6/7'] = {'IB': []}
    for line in ['6', '7']:
        if line in arrivals_nearby:
            arrivals_nearby['6/7']['IB'].extend(arrivals_nearby[line].get('IB', []))
            del arrivals_nearby[line]['IB']
...
def print_times(arrivals_nearby):
    ...
    :return: printed list
    """
    arrivals_nearby = combine_lines(arrivals_nearby)
    ...
    for line, arrivals in arrivals_nearby_sorted:
        for direction, times in sorted(arrivals.items()):
            ...
            times = sorted([time for time in times if time <= 30])[:3]
            time_str = ', '.join(f'{str(time):>3}' for time in times)
            to_print.append(f'{line:>4} {direction} in {time_str:13} minutes') if time_str else None
...
def trigger_6_7(arrivals_nearby, trigger):
    ...
    if '6/7' not in arrivals_nearby or arrivals_nearby['6/7'] == {'←': []}:
        ...
        print(f'Next 6/7 ← in {", ".join(next_6_7)} minutes' if next_6_7 else f'No 6/7 in the next {window_max} minutes.')
...
def main():
    ...
    parser.add_argument('-dev', action='store_true',
                        help='[opt for -print, -trigger, -display] use shelved data')
